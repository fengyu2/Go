<template>
  <div>
    ...
    <el-dialog title="创建市场经理(仅校长可操作)" :visible.sync="dialogTableVisible">
      <el-form ref="createData" :model="createData" :rules="rulesCreateForm"  label-width="80px" center>
        ...
        <el-form-item label="学校" prop="school">
          <el-input v-model="createData.school" placeholder="学校"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      <el-form  label-width="80px" :model="createData">
        ...
        <el-form-item label="学校">
          <el-input v-model="createData.school"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods:{
      updata:function(){
        ...
        this.$http.put('/marketingManager/'+this.createData.id,data,config) //todo bug 500
          .then(response=>{
    ...
    */
      ...
      loadData: function (no = 1, pageSize = 15, queryData) {
        ...
        this.$http.get("/marketingManager?_no=" + no + str )
          .then(response => {
          ...
          this.errorTips(error.response, '获取市场经理错误');
        })
      },
      ...
      Commit:function (formName,createData) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('/marketingManager',data,config) //todo bug 500
              .then(response=>{
      ...
      //删除员工函数
      Delete:function (key) {
        if (confirm('是否删除市场经理:'+key.name+'?')){
          this.$http.delete('/marketingManager/'+key.id)
            .then(response=>{
              this.openTips('删除市场经理', '', 'success')
            ...
            console.log(err);
            this.errorTips(err.response, '删除市场经理失败');
          })
        }
      },
      ...
      resetPassword:function(data){
        ...
        this.$http.put('/user/marketingManager/pwd/reset/'+data.id,config)
          .then(response=>{
            this.open3()
          }).catch(err =>{
          console.log(err);
          this.errorTips(err.response, '当前市场经理不存在');
        })
      },
      ...
      getAll:function (no=1,pageSize=100,queryData={}) {
        ...
        let urls=[];
        for (let i=1;i<=num;i++){
          urls.push("/marketingManager?_no=" + i+'&_pageSize='+5 + str)
        }
        ...
        this.$http.all(requests)
          .then((response)=>{
            ...
            document.getElementById("dLink").download = '市场经理信息.xls';
